...
#' 
#' \code{interplot.glmmkin} is a method to calculate conditional coefficient estimates from the results of multilevel (mixed-effects) regression models with interaction terms. 
#' 
...
interplot.glmmkin <- function(m, var1, var2, plot = TRUE, steps = NULL, ci = .95, adjCI = FALSE, hist = FALSE, var2_dt = NA, predPro = FALSE, var2_vals = NULL, point = FALSE, sims = 5000, xmin = NA, xmax = NA, ercolor = NA, esize = 0.5, ralpha = 0.5, rfill = "grey70", ...) {
  ...
  if (factor_v2) {
    ...
    if (is.na(xmin)) 
      xmin <- min(m$X[, var2], na.rm = T)
    if (is.na(xmax)) 
      xmax <- max(m$X[, var2], na.rm = T)
    
    if (is.null(steps)) {
      # steps <- eval(parse(text = paste0("length(unique(na.omit(m$model$", 
      #                                   var2, ")))")))
      steps <- length(unique(na.omit(m$X[, var2])))
    }
  ...
  if (factor_v1) {
    ...
    for (j in 1:(length(eval(parse(text = paste0("m$xlevel$", var1_bk)))) - 
      ...
      for (i in 1:steps) {
        coef$coef1[i] <- mean(m.sims@coef[, match(var1[j + 1], 
                                                  colnames(m$X))] + coef$fake[i] * m.sims@coef[, match(var12[j], 
                                                                                                       colnames(m$X))])
        coef$ub[i] <- quantile(m.sims@coef[, match(var1[j + 1], 
                                                   colnames(m$X))] + coef$fake[i] * m.sims@coef[, match(var12[j], 
                                                                                                        colnames(m$X))], 1 - (1 - ci) / 2)
      ...
      # min_sim <- m.sims@coef[, match(var1[j + 1], colnames(m$X))] + xmin * m.sims@coef[, match(var12[j], colnames(m$X))] # simulation of the value at the minimum value of the conditioning variable
      # max_sim <- m.sims@coef[, match(var1[j + 1], colnames(m$X))] + xmax * m.sims@coef[, match(var12[j], colnames(m$X))] # simulation of the value at the maximum value of the conditioning variable
    ...
    for (j in 1:(length(eval(parse(text = paste0("m$xlevel$", var2_bk)))) - 
      ...
      for (i in 1:steps) {
        coef$coef1[i] <- mean(m.sims@coef[, match(var1, colnames(m$X))] + 
                                coef$fake[i] * m.sims@coef[, match(var12[j], colnames(m$X))])
        coef$ub[i] <- quantile(m.sims@coef[, match(var1, colnames(m$X))] + 
                                 coef$fake[i] * m.sims@coef[, match(var12[j], colnames(m$X))], 
                               1 - (1 - ci) / 2)
        coef$lb[i] <- quantile(m.sims@coef[, match(var1, colnames(m$X))] + 
                                 coef$fake[i] * m.sims@coef[, match(var12[j], colnames(m$X))], 
                               (1 - ci) / 2)
      }
